...
class PortfolioController extends AbstractActionController {
    ...
    public function portfolioAction() {
        ...
        $view->setTemplate('quadro/portfolio');
        return $view;
    }
        ...
        if ($request->isPost()) {
            ...
            if ($arquivo) {
                if ($arquivo['error'] == 0) {
                    ...
                    $dir = $_SERVER['DOCUMENT_ROOT'] . '/arquivos/portfolio/';

                    if (!file_exists($dir)) {
                        mkdir($dir, 0777);
                    }

                    $params['arquivo_nome'] = 'portfolio-' . date("Y-m-d-H-m-s") . '.' . $ext;
                    ...
                    if (file_exists($destino)) {
                        ...
                        $sql = "insert into portfolio(cod_usuario_fk,conteudo,assunto_fk,arquivo,data_upload) values (:usuario,:conteudo,:assunto,:arquivo_nome,now());";
    ...
    public function deleteAction() {
        ...
        $sql = "select * from portfolio where cod =:cod";
        $result = $funcoes->executarSQL($sql, $params, '');

        $sql = "delete from portfolio where cod =:cod";
        $funcoes->executarSQL($sql, $params);

        unlink($_SERVER['DOCUMENT_ROOT'] . '/arquivos/portfolio/' . $result['arquivo']);
    ...
    public function editAction() {
        ...
        if ($arquivo) {
            if ($arquivo['error'] == 0) {
                ...
                if (file_exists($destino)) {
                    $sql = "update portfolio set conteudo =:conteudo,assunto_fk =:assunto,arquivo=:arquivo_nome where cod =:cod";
                    $funcoes->executarSQL($sql, $params);
            ...
            $sql = "update portfolio set conteudo =:conteudo, assunto_fk =:assunto where cod =:cod";
            $funcoes->executarSQL($sql, $params);
